...
public class NametagsC extends Module {
  ...
  private final Setting<Boolean> reverseArmor = register(new Setting("ReverseArmor", Boolean.valueOf(false)));
  
  private final Setting<Boolean> topEnchant = register(new Setting("TopEnchant", Boolean.valueOf(false)));
  
  private final Setting<Boolean> altEnchantNames = register(new Setting("AltEnchantNames", Boolean.valueOf(false)));
  ...
  private final Setting<Boolean> outline = register(new Setting("Outline", Boolean.FALSE, v -> ((Boolean)this.rect.getValue()).booleanValue()));
  
  private final Setting<Boolean> colorSync = register(new Setting("Sync", Boolean.FALSE, v -> ((Boolean)this.outline.getValue()).booleanValue()));
  ...
  private void renderProperNameTag(EntityPlayer player, double x, double y, double z, float delta) {
    ...
    if (((Boolean)this.rect.getValue()).booleanValue()) {
      ...
      if (((Boolean)this.outline.getValue()).booleanValue()) {
        int color = ((Boolean)this.colorSync.getValue()).booleanValue() ? ClickGui.getInstance().getColor() : (new Color(((Integer)this.redSetting.getValue()).intValue(), ((Integer)this.greenSetting.getValue()).intValue(), ((Integer)this.blueSetting.getValue()).intValue(), ((Integer)this.alphaSetting.getValue()).intValue())).getRGB();
    ...
    if (renderMainHand.hasEffect() && !(renderMainHand.getItem() instanceof net.minecraft.item.ItemTool))
    ...
    ArrayList<?> armorInventory = new ArrayList((Collection<?>)player.inventory.armorInventory);
    ...
    if (renderOffhand.hasEffect() && !(renderOffhand.getItem() instanceof net.minecraft.item.ItemTool))
      renderOffhand.getItem(); 
    renderItemStack(player, renderOffhand, xOffset, -26, ((Boolean)this.armor.getValue()).booleanValue());
    xOffset += 16;
    for (ItemStack stack : player.inventory.armorInventory) {
      if (stack == null)
        continue; 
      ItemStack armourStack = stack.copy();
      if (armourStack.hasEffect() && !(armourStack.getItem() instanceof net.minecraft.item.ItemTool))
        armourStack.getItem(); 
      renderItemStack(player, armourStack, xOffset, -26, ((Boolean)this.armor.getValue()).booleanValue());
      xOffset += 16;
    } 
    renderItemStack(player, renderMainHand, xOffset, -26, ((Boolean)this.armor.getValue()).booleanValue());
  ...
  private void renderItemStack(EntityPlayer player, ItemStack stack, int x, int y, boolean item) {
    ...
    GlStateManager.disableDepth();
    renderEnchantmentText(player, stack, x, y);
    GlStateManager.enableDepth();
  ...
  private void renderEnchantmentText(EntityPlayer player, ItemStack stack, int x, int y) {
    int enchantmentY = (int)((y - 8) - (((Boolean)this.topEnchant.getValue()).booleanValue() ? (getBiggestArmorTag(player) - getEnchantHeight(stack)) : 0.0F));
    ...
    for (int index = 0; index < enchants.tagCount(); index++) {
      ...
      if (enc != null) {
        String string = enc.isCurse() ? (((Boolean)this.altEnchantNames.getValue()).booleanValue() ? enc.getTranslatedName(level).substring(11).substring(0, ((Boolean)this.altEnchantNames.getValue()).booleanValue() ? 3 : 1) : (TextFormatting.RED + enc.getTranslatedName(level).substring(11).substring(0, ((Boolean)this.altEnchantNames.getValue()).booleanValue() ? ((enc.getMaxLevel() == 1) ? 3 : 2) : 1))) : (encName = enc.getTranslatedName(level).substring(0, ((Boolean)this.altEnchantNames.getValue()).booleanValue() ? ((enc.getMaxLevel() == 1) ? 3 : 2) : 1));
        if (enc.getMaxLevel() != 1)
          encName = encName + level; 
        if (!((Boolean)this.altEnchantNames.getValue()).booleanValue())
