...
public class Strafe extends Module {
  ...
  private final Setting<Boolean> inwater = register(new Setting("AntiWater", Boolean.valueOf(true)));
  ...
  private final Setting<Integer> yOffset = register(new Setting("HopHeight", Integer.valueOf(400), Integer.valueOf(350), Integer.valueOf(500)));
  ...
  private final Setting<Integer> step = register(new Setting("SetStep", Integer.valueOf(1), Integer.valueOf(0), Integer.valueOf(2), v -> (this.mode.getValue() == Mode.CWalk)));
  ...
  public void onMove(MoveEvent event) {
    ...
    if (mc.player.isSneaking())
      return; 
    if (mc.player.isInWater() && ((Boolean)this.inwater.getValue()).booleanValue())
      return; 
    if (!mc.player.onGround) {
    ...
    if (this.mode.getValue() == Mode.NCP) {
      ...
      if (((Boolean)this.limiter2.getValue()).booleanValue() && mc.player.onGround && OyVey.speedManager.getSpeedKpH() < ((Float)this.speedLimit2.getValue()).floatValue())
        this.stage = 2; 
      ...
      if (this.stage == 1 && EntityUtil.isMoving()) {
        ...
        if (mc.world.getCollisionBoxes((Entity)mc.player, mc.player.getEntityBoundingBox().offset(0.0D, mc.player.motionY, 0.0D)).size() > 0 || (mc.player.collidedVertically && this.stage > 0))
          this.stage = (((Boolean)this.bhop2.getValue()).booleanValue() && OyVey.speedManager.getSpeedKpH() >= ((Float)this.speedLimit.getValue()).floatValue()) ? 0 : ((mc.player.moveForward != 0.0F || mc.player.moveStrafing != 0.0F) ? 1 : 0); 
  ...
  public void onPacketReceive(PacketEvent.Receive event) {
    if (event.getPacket() instanceof net.minecraft.network.play.server.SPacketPlayerPosLook && ((Boolean)this.noLag.getValue()).booleanValue())
      this.stage = (this.mode.getValue() == Mode.CWalk && (((Boolean)this.limiter2.getValue()).booleanValue() || ((Boolean)this.bhop2.getValue()).booleanValue())) ? 1 : 4; 
  }
