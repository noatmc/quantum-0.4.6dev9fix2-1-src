...
public class GodModule extends Module {
  public Setting<Integer> rotations = register(new Setting("Spoofs", Integer.valueOf(20), Integer.valueOf(1), Integer.valueOf(20)));
  ...
  public void onSendPacket(PacketEvent.Send event) {
    if (event.getStage() == 0 && event.getPacket() instanceof CPacketPlayerTryUseItemOnBlock) {
      ...
      if (mc.player.getHeldItem(packet.hand).getItem() instanceof net.minecraft.item.ItemEndCrystal) {
        if ((((Boolean)this.checkPos.getValue()).booleanValue() && !BlockUtil.canPlaceCrystal(packet.position, ((Boolean)this.entitycheck.getValue()).booleanValue(), ((Boolean)this.oneDot15.getValue()).booleanValue())) || checkPlayers())
          return; 
        updateEntityID();
  ...
  public void rotateTo(BlockPos pos) {
    float[] angle = MathUtil.calcAngle(mc.player.getPositionEyes(mc.getRenderPartialTicks()), new Vec3d((Vec3i)pos));
    this.yaw = angle[0];
    this.pitch = angle[1];
    this.rotating = true;
  }
