...
public class BindButton extends Button {
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    if (this.isListening) {
      OyVey.textManager.drawStringWithShadow("Listening...", this.x + 2.3F, this.y - 1.7F - OyVeyGui.getClickGui().getTextOffset(), getState() ? -1 : -5592406);
    } else {
      OyVey.textManager.drawStringWithShadow(this.setting.getName() + " " + ChatFormatting.GRAY + this.setting.getValue().toString(), this.x + 2.3F, this.y - 1.7F - OyVeyGui.getClickGui().getTextOffset(), getState() ? -1 : -5592406);
    } 
  }
  ...
  public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
    ...
    if (isHovering(mouseX, mouseY))
      Minecraftable.mc.getSoundHandler().playSound((ISound)PositionedSoundRecord.getMasterRecord(SoundEvents.UI_BUTTON_CLICK, 1.0F)); 
  }
