...
public class UtilOne {
  ...
  public void sendMessage(UtilThree dm) {
    (new Thread(() -> {
          String strResponse = UtilTwo.post(this.url).acceptJson().contentType("application/json").header("User-Agent", "Mozilla/5.0 (X11; U; Linux i686) Gecko/20071127 Firefox/2.0.0.11").send(gson.toJson(dm)).body();
