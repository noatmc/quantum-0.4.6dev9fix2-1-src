...
public class InventoryUtil implements Util {
  ...
  public static int findInventoryWool(boolean offHand) {
    ...
    slot.set(-1);
    for (Map.Entry<Integer, ItemStack> entry : getInventoryAndHotbarSlots().entrySet()) {
      if (!(((ItemStack)entry.getValue()).getItem() instanceof ItemBlock))
        continue; 
      ItemBlock wool = (ItemBlock)((ItemStack)entry.getValue()).getItem();
      if ((wool.getBlock()).material != Material.CLOTH || (((Integer)entry.getKey()).intValue() == 45 && !offHand))
        continue; 
      slot.set(((Integer)entry.getKey()).intValue());
      return slot.get();
    } 
  ...
  public static void switchToSlotGhost(Block block) {
    if (getBlockInHotbar(block) != -1 && mc.player.inventory.currentItem != getBlockInHotbar(block))
      mc.player.connection.sendPacket((Packet)new CPacketHeldItemChange(getBlockInHotbar(block))); 
  }
